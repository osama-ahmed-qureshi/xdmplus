...
public class BrowserMonitor implements Runnable {
	...
	public void updateSettingsAndStatus() {
		try {
			Path homePath = Paths.get(System.getProperty("user.home"), ".xdmplus");
			if (!Files.exists(homePath)) {
				Files.createDirectories(homePath);
			}
			Files.writeString(Paths.get(System.getProperty("user.home"), ".xdmplus", "settings.json"), getSync());
			Files.writeString(Paths.get(System.getProperty("user.home"), ".xdmplus", "settings_updated"), "");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
