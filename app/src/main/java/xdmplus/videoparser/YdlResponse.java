...
import xdmplus.network.http.HttpHeader;
...
public class YdlResponse {
	...
	public static ArrayList<YdlVideo> parse(InputStream in) throws Exception {
		JsonObject obj = JsonParser.parseReader(new InputStreamReader(in, "utf-8")).getAsJsonObject();
		JsonArray entries = getArray(obj, "entries");
		if (entries == null) {
			// its a playlist
		...
		for (int i = 0; i < entries.size(); i++) {
			JsonObject jsobj = asObject(entries.get(i));
			if (jsobj != null) {
	...
	public static YdlVideo getPlaylistEntry(JsonObject obj) {
		...
		JsonArray formats = getArray(obj, "formats");
		if (formats != null) {
			for (int i = 0; i < formats.size(); i++) {
				Logger.log("Parsing format info");
				JsonObject formatObj = asObject(formats.get(i));
				...
				String sabr = rawString(formatObj.get("abr"));
				try {
				...
				JsonObject jsHeaders = asObject(formatObj.get("http_headers"));
				if (jsHeaders != null) {
					format.headers = new ArrayList<>();
					Iterator<String> headerIter = jsHeaders.keySet().iterator();
					while (headerIter.hasNext()) {
						String key = headerIter.next();
						String value = getString(jsHeaders.get(key));
						format.headers.add(new HttpHeader(key, value));
					}
				}
				if (protocol.equals("http_dash_segments")) {
					String baseUrl = getString(formatObj.get("fragment_base_url"));
					JsonArray fragmentArr = getArray(formatObj, "fragments");
					String[] fragments = new String[fragmentArr.size()];
					for (int j = 0; j < fragmentArr.size(); j++) {
						JsonObject frag = asObject(fragmentArr.get(j));
						String url = getString(frag.get("url"));
						if (url != null) {
							fragments[j] = url;
						} else {
							fragments[j] = baseUrl + getString(frag.get("path"));
						}
					}
			...
			if (url != null) {
				...
				String sabr = rawString(obj.get("abr"));
				try {
					format.abr = Integer.parseInt(sabr);
				} catch (Exception e) {
					format.abr = -1;
		...
		Logger.log("VIDEO----" + getString(obj.get("title")));
		...
		String stitle = getString(obj.get("title"));
		if (!StringUtils.isNullOrEmptyOrBlank(stitle)) {
			pl.title = stitle;
		}

		String thumbnail = getString(obj.get("thumbnail"));
		if (thumbnail != null) {
		...
		if (pl.thumbnail == null) {
			JsonArray thumbnails = getArray(obj, "thumbnails");
			if (thumbnails != null) {
				for (int i = 0; i < thumbnails.size(); i++) {
					Logger.log("Parsing thumbnails info");
					JsonObject thumbnailObj = asObject(thumbnails.get(i));
					thumbnail = getString(thumbnailObj.get("url"));
					if (thumbnail != null) {
						if (!(thumbnail.equals("none") || thumbnail.equals("null"))) {
							pl.thumbnail = thumbnail;
							break;
		...
		String sdur = rawString(obj.get("duration"));
		if (sdur != null) {
	...
	private static String rawString(JsonElement el) {
		if (el == null || el.isJsonNull()) {
			return "null";
		}
